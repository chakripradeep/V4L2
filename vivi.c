...
//	v4l2_get_timestamp(&buf->vb.vb2_buf.timestamp);
	...
	dprintk(dev, 1, "\n%s %d ##### now, bufer state is changed to VB2_BUF_STATE_DONE index: %d\n",__func__,__LINE__,buf->vb.vb2_buf.index);

	vb2_buffer_done(&buf->vb.vb2_buf, VB2_BUF_STATE_DONE);
	printk(KERN_INFO "%s[%d] $$ [%p/%d] done \n",__FUNCTION__,__LINE__,buf, buf->vb.vb2_buf.index);
...
static void vivi_stop_generating(struct vivi_dev *dev)
{
	...
	while (!list_empty(&dma_q->active)) {
		...
		buf = list_entry(dma_q->active.next, struct vivi_buffer, list);
		list_del(&buf->list);
		dprintk(dev, 1, "\n%s %d ##### now, bufer state is changed to VB2_BUF_STATE_ERROR index: %d \n",__func__,__LINE__,buf->vb.vb2_buf.index);
...
static int buffer_prepare(struct vb2_buffer *vb)
{
	...
	struct vivi_buffer *buf = container_of(vbuf, struct vivi_buffer, vb);
	unsigned long size;

	dprintk(dev, 1, "\n%s %d, field=%d\n", __func__,__LINE__, vbuf->field);
...
static int start_streaming(struct vb2_queue *vq, unsigned int count)
{
	...
	printk(KERN_INFO "%s[%d] err:%d\n",__FUNCTION__,__LINE__,err);
	if (err) {
		struct vivi_buffer *buf, *tmp;

		list_for_each_entry_safe(buf, tmp, &dev->vidq.active, list) {
			list_del(&buf->list);
			dprintk(dev, 1, "\n%s %d ##### now, bufer state is changed to VB2_BUF_STATE_QUEUED , buffer index :%d\n",__func__,__LINE__,buf->vb.vb2_buf.index);
